...
dirc = params['dirc']
fil_save_name += '_ring'
fil_save_name += '_L%d_'%L1
fil_save_name +=str(randint(1, 1000))
...
for No in No_list:
    ...
    for i in range(steps):
       # data['z'].append(measure_z(state_evolve))
        ...
        H = (H_V+func_Omega((i)*dt)*H_O+func_Delta((i)*dt)*H_D)
        data['t'].append((i)*dt)
        data['Detuning'].append(func_Delta((i)*dt)/2/np.pi)
        eigvals,eigvecs = H.eigsolve(getvecs=True,nev = 10)
     #   print(eigvals)
        data['E0'].append(eigvals[0])
        data['E1'].append(eigvals[1])
        
        data['E2'].append(eigvals[2])
        data['E3'].append(eigvals[3])
   ...
   #     print(data_temp)
        np.savetxt(path.join(dirc,'gap'+fil_save_name), data_temp,delimiter=',')
    for ob in op_list2:
        #print()
        np.savetxt(path.join(dirc,ob+fil_save_name), data[ob], delimiter=',')
print('done')
print(fil_save_name)
