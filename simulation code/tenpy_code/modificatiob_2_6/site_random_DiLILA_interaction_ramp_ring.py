...
def build_hamiltonian_interaction(pos_lst,site_dep_int=np.zeros(N)):
    h = 0*sigmaz(0)
    length = len(pos_lst)
    #print(length)
    for site1 in range(0,length):
        ...
        for site2 in range(site1+1,length):
            randomfactor = (site_dep_interaction[sites1]+ site_dep_interaction[sites2])/2
            ...
            h += (1+randomfactor)*func_V(distance)*(sigmaz(site1)+identity())*(sigmaz(site2)+identity())/4
...
def build_hamiltonian_drive(Omega=1,site_dep_rabi=np.zeros(N)):
    H = 0*sigmax(0)
    for site in range(N):
        H += (1+site_dep_rabi[site])*Omega/2*sigmax(site)
    return H

#def build_hamiltonian_rabi_random(Omega=1,site_dep_rabi):
#    H = 0*sigmax(0)
#    for site in range(N):
#        H += (1+site_dep_rabi[site])*Omega/2*sigmax(site)
#    return H
...
for No in No_list:
    if OBC: 
        ...
        print(pos_lst)
    site_dep_int= 0.5* (np.random.random(N)-0.5)
    H_V = build_hamiltonian_interaction(pos_lst[pos_occupied],site_dep_int = site_dep_int)
    ...
    data['zg'] = []
    data['zzg'] = []
    data['site_dep_int'] = site_dep_int
