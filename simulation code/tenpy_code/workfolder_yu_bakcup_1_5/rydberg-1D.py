        ...
        for pairs in self.lat.pairs.values():
            for u1, u2, dx in pairs:
                ...
                if distance > distance_cutoff:
                    continue
                cord_distance = radius*np.sin(2*np.pi/L*distance)
...
product_state = [["up"]]
psi0 = MPS.from_lat_product_state(M.lat, product_state) # the initial state, which we take the overlap with
...
if rerun == True:
    ...
    data = {'z':[],'zz':[],'zg':[],'zzg':[],'overlap':[],'E':[],'nT':0}
    nT = 0

def measure(states):
    data['z'].append(psi.expectation_value('P1'))
    data['zz'].append(psi.correlation_function('P1','P1'))
    psi_gs,psi_ex1,psi_ex2 = states
    data['zg'].append(psi_gs.expectation_value('P1'))
    data['zzg'].append(psi_gs.correlation_function('P1','P1'))
    data['zzg'].append(np.array([float(abs(psi.overlap(psi_gs))**2),float(abs(psi.overlap(psi_ex1))**2),float(abs(psi.overlap(psi_ex2))**2)]))
    data['E'].append(E)
