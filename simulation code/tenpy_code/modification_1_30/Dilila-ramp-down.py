...
def Omegafunc(time):
    ...
    if time > Rabi_ramp_time+ T_DiLILA:
        return Omega*(1-(time - Rabi_ramp_time- T_DiLILA )/0.048)
    else:
        return Omega
...
tdvp_params_evolve = {
        ...
        'dt': dt/N_steps,
        'N_steps': N_steps,
        'start_time': T_DiLILA+Rabi_ramp_time,
        'max_sweeps': 2,
        'compression_method': 'variational'
    }
...
N_times_DiLILA = int(T_DiLILA/dt)
N_times_rampdown = int(0.048/0.001)
file_name += 'Dilila-rampdown'
for n in range(0,N_times_rabi):
    ...
    eng_tdvp.run()
eng_tdvp = tdvp.TimeDependentTwoSiteTDVP(psi, M, tdvp_params_DiLILA)
for n in range(N_times_rabi,N_times_DiLILA+N_times_rabi):
    psi_gs = MPS.from_lat_product_state(eng_tdvp.model.lat, product_state)
    psi_ex1 = MPS.from_lat_product_state(eng_tdvp.model.lat, product_state)
    psi_ex2 = MPS.from_lat_product_state(eng_tdvp.model.lat, product_state)
    ...
    measure()
    np.save(dirc+file_name+".npy",data)
    print('DiLILA Ramp, Run step: '+str(n-N_times_rabi)+'total '+ str(N_times_DiLILA))
    eng_tdvp.run()
    
eng_tdvp = tdvp.TimeDependentTwoSiteTDVP(psi, M, tdvp_params_evolve)
for n in range(N_times_DiLILA+N_times_rabi,N_times_DiLILA+N_times_rabi+N_times_rampdown):
    psi_gs = MPS.from_lat_product_state(eng_tdvp.model.lat, product_state)
    psi_ex1 = MPS.from_lat_product_state(eng_tdvp.model.lat, product_state)
    psi_ex2 = MPS.from_lat_product_state(eng_tdvp.model.lat, product_state)
    ...
    measure()
    np.save(dirc+file_name+".npy",data)
    print('Ramp down, Run step: '+str(n-N_times_DiLILA-N_times_rabi)+'total '+ str(N_times_rampdown))
    eng_tdvp.run()
