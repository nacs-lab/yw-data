...
for No in No_list:
    ...
    #state_evolve = State(state=('DU'*int(N/2+1))[:N])
    steps = int(T/dt)
    ...
    for i in range(steps):
        ...
        state_evolve = (H_V+func_Omega((i+0.5)*dt)*H_O+func_Delta((i+0.5)*dt)*H_D).evolve(state_evolve, t=dt)
    data['z'].append(measure_z(state_evolve))
    data['zz'].append(measure_zz(state_evolve))
    ...
    np.savetxt(path.join(dirc,'pos_'+fil_name), pos_lst, delimiter=',')
print('done')
