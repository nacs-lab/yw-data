...
config.initialize(['-eps_monitor'],gpu = True)
...
for No in No_list:
    ...
    data = {}
    data['pos_occupied'] = pos_occupied
    data['z'] = []
    data['zz'] = []
    ...
    product_state = State(state='D'*N)
    AFM_state = State(state =( 'UD'*int(N/2+1))[:N])
    if stepsrabi>0:
        ...
        for j in range(stepsrabi):
            ...
            eigvals,eigvecs = H.eigsolve(getvecs=True,nev = 4,max_its = 200)
    ...
    for i in range(steps):
        ...
        eigvals,eigvecs = H.eigsolve(getvecs=True,nev = 4, max_its=500)
    ...
    op_list2 = ['pos_occupied','z','zz','zg','zzg']
