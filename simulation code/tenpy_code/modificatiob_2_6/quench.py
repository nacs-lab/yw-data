...
quench_time = params['quench_time']
#T_DiLILA = params['T_DiLILA']
#gapinit = 2*np.pi* params['Gap_init']
#gapcrit = 2*np.pi* params['Gap_crit']
#gapfin = 2*np.pi* params['Gap_final']
#dcrit = 2*np.pi* params['Delta_crit']
dinit = 2*np.pi*params['Delta_ini']
dfin = 2*np.pi*params['Delta_fin']
quench_dt = params['quench_dt']
evolve_time = params['evolve_time']
dt = params['dt'] # detuning scanning dt
#auto_tinflect = params['auto_tinflect']
#
def Omegafunc(time):
    if time < Rabi_ramp_time:
...
def quench(t,quench_time,dinit,dfin):
    if t>quench_time:
        return dfin
    if t<0:
        return dinit
    else: 
        return (dfin-dinit)*t/quench_time+dinit


def Deltafunc(time):
    t = time - Rabi_ramp_time
    return quench(t,quench_time,dinit,dfin)
...
tdvp_params_evolve = {
        ...
        'dt': dt/N_steps,
        'N_steps': N_steps,
        'start_time': Rabi_ramp_time+quench_time,
        'max_sweeps': 2,
        'compression_method': 'variational'
    }
...
N_times_quench = int(quench_time/quench_dt)
N_times_evolve = int(evolve_time/dt)
for n in range(0,N_times_rabi):
    psi_gs = MPS.from_lat_product_state(eng_tdvp.model.lat, product_state)
    psi_ex1 = MPS.from_lat_product_state(eng_tdvp.model.lat, product_state)
    psi_ex2 = MPS.from_lat_product_state(eng_tdvp.model.lat, product_state)
    ...
    eng_tdvp.run()
eng_tdvp = tdvp.TimeDependentTwoSiteTDVP(psi, M, tdvp_params_quench)
for n in range(N_times_rabi,N_times_quench+N_times_rabi):
    psi_gs = MPS.from_lat_product_state(eng_tdvp.model.lat, product_state)
    psi_ex1 = MPS.from_lat_product_state(eng_tdvp.model.lat, product_state)
    psi_ex2 = MPS.from_lat_product_state(eng_tdvp.model.lat, product_state)
    ...
    measure()
    np.save(dirc+file_name+".npy",data)
    print('Detuning quench, Run step: '+str(n-N_times_rabi)+'total '+ str(N_times_quench))
    eng_tdvp.run()

eng_tdvp = tdvp.TimeDependentTwoSiteTDVP(psi, M, tdvp_params_evolve)

for n in range(N_times_rabi+N_times_quench,N_times_quench+N_times_rabi+N_times_evolve):
    psi_gs = MPS.from_lat_product_state(eng_tdvp.model.lat, product_state)
    psi_ex1 = MPS.from_lat_product_state(eng_tdvp.model.lat, product_state)
    psi_ex2 = MPS.from_lat_product_state(eng_tdvp.model.lat, product_state)
    ...
    measure()
    np.save(dirc+file_name+".npy",data)
    print('Evolve, Run step: '+str(n-N_times_rabi-N_times_quench)+'total '+ str(N_times_evolve))
    eng_tdvp.run()
